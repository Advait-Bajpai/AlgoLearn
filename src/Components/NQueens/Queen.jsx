...
class Queen extends Component {
  ...
  queensAlgo = async (board, col) => {
    if (col >= this.state.number) return true;
  ...
  render() {
    return (
      <div>
        ...
        <div className="board-container pt-24 flex justify-center items-center">
...
const getChecked = (board, row, col, N) => {
  ...
  for (let i = 0; i < N; i++) {
    if (newBoard[row][i].isPresent) {
      ...
      pos = false;
    } else newBoard[row][i] = { ...newBoard[row][i], isChecked: true };
  }
  ...
  for (let i = 0; i < N; i++) {
    if (newBoard[i][col].isPresent) {
      ...
      pos = false;
    } else newBoard[i][col] = { ...newBoard[i][col], isChecked: true };
  }
